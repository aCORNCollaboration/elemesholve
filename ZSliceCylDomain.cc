...
#include "ZSliceCylDomain.hh"

ZSliceCylDomain::ZSliceCylDomain(const CoordinateTransform* CT): GeomDomainFunction(CT) {
    zslices[-world_dz] = NULL;
    zslices[world_dz] = NULL;
}

const GeomSubVolume* ZSliceCylDomain::findPoint(double x, double y, double z) const {
...
int ZSliceCylDomain::f(double x, double y, double z) const {
...
double ZSliceCylDomain::meshsize(double x, double y, double z) const {
    const GeomSubVolume* V = findPoint(x,y,z);
    if(V) {
...
void ZSliceCylDomain::add_features(Polylines& v) const {
...
void ZSliceCylDomain::addLayer(GeomSubVolume* V) {
    assert(V);
    zslices[V->myBounds.zmin()] = NULL;
    zslices[V->myBounds.zmax()] = V;
}

void ZSliceCylDomain::display() const {
...
GeomSubVolume::GeomSubVolume(ZSliceCylDomain* W, GeomSubVolume* P):
parent(P), theWorld(W) {
...
GeomSphereFunction::GeomSphereFunction(ZSliceCylDomain* W, double rsquared, double xc, double yc, double zc, GeomSubVolume* P):
...
GeomTorusFunction::GeomTorusFunction(ZSliceCylDomain* W, double RR, double rr, GeomSubVolume* P):
...
GeomXSliceBox::GeomXSliceBox(ZSliceCylDomain* W, const CGAL::Bbox_3& B, GeomSubVolume* P):
GeomSubVolume(W,P) {
    myLabel = 1;
    myBounds = B;
...
GeomYRod::GeomYRod(ZSliceCylDomain* W, double xc, double yc, double zc, double ly, double rxy, GeomSubVolume* P):
...
GeomRing::GeomRing(ZSliceCylDomain* W, double rin, double rout, double zmn, double zmx, GeomSubVolume* P):
